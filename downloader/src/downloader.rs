...
pub fn images_json_url() -> String {
    let default_url = "https://images.cdn.convoypanel.com/images.json".to_string();
    // First non-flag positional arg is the images list URL
    ...
    if let Ok(env_url) = env::var("CONVOY_IMAGES_JSON") {
...
pub async fn fetch_templates() -> anyhow::Result<Vec<Group>> {
    let url = images_json_url();
    eprintln!("Using images list: {}", url);
