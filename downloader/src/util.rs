...
pub fn get_storage_location() -> String {
    // Prefer non-interactive override (env or --storage=…)
    if let Ok(env_storage) = std::env::var("CONVOY_STORAGE") {
        if !env_storage.trim().is_empty() {
            return env_storage;
        }
    }
    for arg in std::env::args().skip(1) {
        if let Some(val) = arg.strip_prefix("--storage=") {
            if !val.is_empty() {
